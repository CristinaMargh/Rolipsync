...
"""
Estimate visual timestamps for known words on a selected video ROI.

This script is used for assisted annotation. It does not perform free vocabulary
recognition. For each transcript segment, it takes the set of candidate words,
slides a temporal window over the selected ROI, scores each word using CTC loss,
and returns the best timestamp candidate.
"""
...
def import_train_module(train_py):
    spec = importlib.util.spec_from_file_location("train_ctc_mod", train_py)
    mod = importlib.util.module_from_spec(spec)
    sys.modules["train_ctc_mod"] = mod
...
def main():
    ...
    ap.add_argument("--seg_pad", type=float, default=5.0)
    ...
    for _, seg in tqdm(seg_df.iterrows(), total=len(seg_df)):
        ...
        for ws, we in windows:
            vt, vl = crop_video_window(args.video, ws, we, bbox, args.target_h, args.target_w, args.max_frames, args.fps)
            if vt is not None:
        ...
        for word in words:
            ...
            for ws, we, vt, vl in win_data:
                score, loss = ctc_score_word(model, tok, word, vt, vl, device=device, score_mode=args.score_mode)
